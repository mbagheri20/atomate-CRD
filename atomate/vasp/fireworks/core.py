...
# import phonon from phonopy database
ph=phonopy.load("phonon.yaml", primitive_matrix='auto')
phonon_cell = ase.io.read(filename='POSCAR-unitcell')
atoms_masses = phonon_cell.get_masses()
matp_id =open("id.txt", "rt").read()

#store freq and evs
[freqs,evs] = ph.get_frequencies_with_eigenvectors(q=[0,0,0])
ph_evals = (freqs / 15.633302153867165)**2
phonon_evals = -np.copysign(ph_evals, freqs)
evt = evs.transpose()
real_evs = evt.real
natoms = int((len(freqs))/3)
#mama = np.tile(np.sqrt(atoms_masses),(3,1)).transpose()
phonon_eigenvectors = []
for ev in real_evs:
                ...
                pass_vasp_result(
                    {
                        ...
                        "eigenvals": phonon_evals,
                        "eigenvecs": phonon_eigenvectors,
                        "mpid": matp_id,
                    },
                    parse_eigen=True,
